...
iu tau time = iu' 0 i0 uc0
    where iu' time' i uc | time'>time = [[time',i,uc,rp i,p (tz i)]]
                         | otherwise  = [time',i,uc,rp i,p (tz i)]:(iu' (time' + tau) i' uc')
...
test = do bracket (openFile "result\\iur.txt" WriteMode)
                  hClose
                  (\h -> do mapM_ (\x -> hPutStrLn h (foldl (\s a -> s++(show a)++"\t") "" x)) (iu 2e-6 350e-6))
          bracket (openFile "result\\iu.txt" WriteMode)
                  hClose
                  (\h -> do mapM_ (\x -> hPutStrLn h (foldl (\s a -> s++(show a)++"\t") "" x)) (iu' 5e-6 350e-6))
